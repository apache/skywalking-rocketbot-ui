    ...
    <div class="rk-dashboard-bar flex-h">
      <span class="flex-h">
        ...
        <div class="rk-dashboard-bar-btn">
          <span v-tooltip:bottom="{ content: 'import' }">
            ...
            <label class="rk-btn ghost input-label" for="service-file">
      ...
      <div class="rk-dashboard-bar-tool flex-h">
        ...
        <DashboardEvent
          ...
          :type="pageEventsType.TOPO_SERVICE_EVENTS"
        />
      </div>
    </div>
...
<script lang="ts">
  ...
  import ServicesSurvey from './service-survey.vue';
  ...
  export default class WindowService extends Vue {
    @Prop() private current!: { key: number | string; label: number | string };
    @Prop() private serviceComps: any;
    ...
    private async importData(event: any) {
      try {
        ...
        this.$emit('changeServiceComps', { json: data, type: ObjectsType.UPDATE_SERVICES });
        const el: any = document.getElementById('service-file');
        el!.value = '';
      } catch (e) {
    ...
    private exportData() {
      const data = this.serviceComps;
      const name = 'serviceComps.json';
      saveFile(data, name);
    }

    private beforeDestroy() {
      this.SET_EDIT(false);
      this.$emit('changeServiceComps', { type: '' });
    }
  }
</script>
