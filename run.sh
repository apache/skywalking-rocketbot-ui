...
pool_name2="skywalkingCommon"

# mv ${absolute_path} ${absolute_path}.old
# cat ${absolute_path}.old | sed s#SKYWALKING_URL#${SKYWALKING_URL}#g  >  ${absolute_path}

skywalking_collector2=${SKYWALKING_COMMON}

#skywalking_collector=localhost:1234,127.0.0.1:1235

 #pool对应ip地址列表,多个ip以逗号改开
pool_ip2=`awk 'BEGIN{list="'${skywalking_collector2}'";split(list,ip_list,",");for(ip in ip_list){print ip_list[ip];}}'`
for ip in ${pool_ip2[*]};do
    echo "add ${pool_name2} ${ip} in ${ngx_upstream_file}"
    sed -i '/upstream '${pool_name2}'[^-]*{/a\\server '${ip}';' ${ngx_upstream_file}
done
